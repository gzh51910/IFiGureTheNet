...
Router.get('/findAll', async(req, res) => {
        // mongodb查询数据库（1）
    let data = await mongodb.Find('OsaUser');
    res.send(formatData({
        data
    }))
})
...
Router.get('/findOne', async(req, res) => {
    ...
    // mongodb查询数据库（1）
    let data = await mongodb.findOne('OsaUser', { name });
    res.send(formatData({
        data
    }));
})
...
Router.post('/addAll', async(req, res) => {
    ...
    console.log(num,
        ...
        time,
        price);
    let result = await mongodb.Create('OsaUser', {
        num,
        name,
        time,
        price
...
Router.get('/removeuser', async(req, res) => {
    ...
    console.log(value);
    // 查询数据库
    let result = await mongodb.Remove('OsaUser', {
        _id: value
    });
    if (result.deletedCount > 0) {
        res.send(formatData())
...
Router.patch('/update', async(req, res) => {
    ...
    console.log(id,num,name,price,time);
